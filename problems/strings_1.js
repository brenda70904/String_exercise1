/*
  npm test strings_1 去檢驗看題目寫得對不對。
*/
...
*/
function lowerCombine(c,d){
  let lowercombine = c+d
  return lowercombine.toLowerCase();
}
...
*/
function makeCombineReversed(a, b, c){
  return c+b+a
}
...
function makeOutWord(a, b) {
 ...
 //return a.substr(0,a.length/2)+ b + a.substr(a.length/2,a.length/2);
return a.substring(0,a.length/2) + b + a.substring(a.length/2, a.length)
}
...
*/
function extraEnd(a){
  return a.slice(-2) + a.slice(-2) +a.slice(-2)
  // let result = "";
  // for(let i = 0; i <3 ; i++){
  //   result= result + a.slice(-2)
  // }
  // return result;
  }  
...
/*
  ...
  getOddChar("H") -> "H"
*/
function getOddChar(word){
  let result ="";
  for(let i= 0; i < word.length; i=i+2){
    result = result + word.substr(i,1)
  }
return result;
}
...
function nonStart(a, b){
    let combine = a.substr(1, a.length)+ b.substr(1, b.length);
    return combine;
...
function middleTwo(a){
  if(a.length % 2 === 0 ){
    return a.substr(a.length/2 - 1, 2)
  }else{
    return""
  }
}
...
function middleThree(a){
  return a.substr(a.length/2 -1, 1 ) + a.substr(a.length/2, 1) + a.substr(a.length/2 + 1, 1);
}
...
function seeBlue(a){
    ...
    for(i = 0; i <= a.length-4; i++){
      //a.substr(i,4)
      if(a.substring(i,i+4) === text){
        return true;
      } 
    }
    return false;
    //JavaScript => forloop, for...in, for...of , map, filter, reduce, foreach
    //Java => forloop, foreach, Iterator class 
  }
/*-----------------------------------
